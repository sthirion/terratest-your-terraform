...
func TestTerraformAwsS3(t *testing.T) {
	...
	// in your AWS account
	expectedName := fmt.Sprintf("s3-bucket-%s", strings.ToLower(random.UniqueId()))
	...
	actualStatus := aws.GetS3BucketVersioning(t, awsRegion, expectedName)
	expectedStatus := "Enabled"
	assert.Equal(t, expectedStatus, actualStatus)

	// Verify that our Bucket has a policy attached
	aws.AssertS3BucketPolicyExists(t, awsRegion, expectedName)
